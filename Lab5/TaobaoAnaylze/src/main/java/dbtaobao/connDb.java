...
public class connDb {
	...
	public static ArrayList index_2() throws SQLException {
		...
		rs = stmt.executeQuery(
				"select gender,age_range,count(*) num from user_log where gender<>'2' group by gender,age_range order by num desc");
		while (rs.next()) {
	...
	public static ArrayList index_4() throws SQLException {
		...
		while (rs.next()) {
			String[] temp = { rs.getString("province").toString(), rs.getString("num").toString() };
			list.add(temp);
		}
		endConn();
		return list;
    ...
    public static ArrayList index_5() throws SQLException{
        ...
        startConn();
        stmt = con.createStatement();
        rs = stmt.executeQuery("select * from (select count(*) positive from rebuy where label='0.0') tt join (select count(*) negative from rebuy where  label='1.0') ttt join (select count(*) num from rebuy) tttt");
        while(rs.next()){
            String[] temp={rs.getString("positive"),rs.getString("negative"),rs.getString("num")};
            list.add(temp);
        }
        endConn();
        return list;
